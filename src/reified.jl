...
function _build_reified_constraint(_error::Function, lhs, rhs)
...
function parse_constraint_call(
    ...
    vectorized, parse_code, build_call = parse_constraint(_error, rhs.args[1])
    if vectorized
        _error("vectorized constraints cannot be used with reification.")
    end
    new_build_call =
