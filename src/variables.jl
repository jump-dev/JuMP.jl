...
```
"""
function has_variable_in_set(x::AbstractJuMPScalar)
    model = owner_model(x)
    return haskey(model.variable_in_set_ref, x)
end

function has_variable_in_set(x::AbstractArray{<:AbstractJuMPScalar})
...
variable was constrained on creation, use [`has_variable_in_set`](@ref).
