...
"""
    _add_kw_args(call, kw_args)::Nothing
...
_is_anonymous(x::Symbol) = startswith(string(x), "##")

_is_anonymous(x::Expr) = Meta.isexpr(x, :vect) || Meta.isexpr(x, :vcat)

# Given a base_name and index_names, returns an expression that constructs the
# name of the object. For use within macros only.
function _name_call(base_name, index_names)
    if isempty(index_names) || base_name == ""
        return base_name
    end
    ...
    for i in 1:length(index_names)
        ...
        push!(ex.args, :(string($(esc(index_names[i])))))
...
###
### Code that is common to all JuMP macros.
###

_valid_model(::AbstractModel, ::Any) = nothing

function _valid_model(m::M, name::Any) where {M}
    return error("Expected $name to be a JuMP model, but it has type $M")
end

"""
    _finalize_macro(model, code, source::LineNumberNode)::Expr
...
function _finalize_macro(model, code, source::LineNumberNode)
    valid_model_check = Expr(:call, _valid_model, model, quot(model.args[1]))
    return Expr(:block, source, valid_model_check, code)
end
...
Return `code` which returns the value of `variable` and then assigns `variable`
to `name`.
...
function _macro_assign_and_return(code, name; model_for_registering = nothing)
    ...
    if model_for_registering !== nothing
        expr = :(_error_if_cannot_register($model_for_registering, $q_name))
        push!(block.args, expr)
    end
    push!(block.args, :($variable = $code))
    if model_for_registering !== nothing
        push!(block.args, :($model_for_registering[$q_name] = $variable))
    end
    # This assignment should be in the scope calling the macro
    push!(block.args, :($(esc(name)) = $variable))
    return block
end

function _finalize_and_return(model, code, source, name = nothing)
    if name === nothing
...
"""
    _check_vectorized(sense::Symbol) --> Tuple{Symbol,Bool}

Given a sense symbol like `:(.>=)` or `:(==)`, return a tuple of the
non-broadcasted Symbol and a Bool indicating if `sense` was broadcasted.
"""
...
function _macro_error(macro_name, args, source, msg...)
...
function _plural_macro_code(m, x, sym)
    if !Meta.isexpr(x, :block)
        error(
            "Invalid syntax for $(sym)s. The second argument must be a " *
            "`begin end` block. For example:\n" *
            "```julia\n@$(sym)s(model, begin\n    # ... lines here ...\nend)\n```.",
        )
    end
    @assert x.args[1] isa LineNumberNode
    last_line = x.args[1]
    code = Expr(:tuple)
    jump_macro = Expr(:., JuMP, QuoteNode(sym))
    for it in x.args
        if it isa LineNumberNode
            last_line = it
        elseif isexpr(it, :tuple)  # Line with commas.
            macro_call = Expr(:macrocall, jump_macro, last_line, m)
            ...
            macro_call = Expr(:macrocall, jump_macro, last_line, m, it)
            push!(code.args, esc(macro_call))
        end
    end
    return code
...
function parse_constraint_call(
    ...
    return _error(
        "Unsupported constraint expression: we don't know how to parse " *
        "constraints containing the operator $op.\n\nIf you are writing a " *
        "JuMP extension, implement " *
        "`parse_constraint_call(::Function, ::Bool, ::Val{$op}, args...)",
    )
end
...
function build_constraint(_error::Function, f, set, args...; kwargs...)
    arg_str = isempty(args) ? "" : ", " * join(args, ", ")
    kwarg_str = join(["$k = $v" for (k, v) in kwargs], ", ")
    kwarg_str = isempty(kwargs) ? "" : "; " * kwarg_str
    return _error(
        "Unrecognized constraint building format. Tried to invoke " *
        "`build_constraint(error, $f, $(set)$(arg_str)$(kwarg_str))`, " *
...
function build_constraint(
    ...
    return _error(
        "Unable to add the constraint because we don't recognize $f as a " *
        "valid JuMP function.",
    )
end
...
function build_constraint(
    ...
    return ScalarConstraint(f, set)
end

function _remove_constant(f::Union{GenericAffExpr,GenericQuadExpr})
    offset = constant(f)
    add_to_expression!(f, -offset)
    return f, offset
end

_remove_constant(f::Number) = zero(f), f

function build_constraint(
    ::Function,
    f::Union{Number,GenericAffExpr,GenericQuadExpr},
    set::MOI.AbstractScalarSet,
)
    if MOI.Utilities.supports_shift_constant(typeof(set))
        f, offset = _remove_constant(f)
        new_set = MOI.Utilities.shift_constant(set, -offset)
        return ScalarConstraint(f, new_set)
    end
    return ScalarConstraint(f, set)
end
...
function build_constraint(
    ::Function,
    f::AbstractVector{<:Union{Number,AbstractJuMPScalar}},
...
function build_constraint(
    ...
    return _error("instead of `$(set)`, use `JuMP.PSDCone()`.")
end

# The three-argument build_constraint is used for two-sided comparison
# constraints.
...
function build_constraint(
    ...
    if isnan(l) || isnan(u)
        _error("Invalid bounds, cannot contain NaN: [$l, $u].")
    end
    T = value_type(variable_ref_type(f))
    return build_constraint(_error, f, MOI.Interval{T}(l, u))
end
...
# This method intercepts `@constraint(model, l <= x <= u)` and promotes
# `x` to an `AffExpr` to form a `ScalarAffineFunction-in-Interval` instead of
# `VariableIndex-in-Interval`. To create a `MOI.VariableIndex`-in-`MOI.Interval`,
# use `@constraint(model, x in MOI.Interval(l, u))`. We do this for consistency
# with how one-sided (in)equality constraints are parsed.
function build_constraint(
    _error::Function,
    f::AbstractVariableRef,
    l::Real,
    u::Real,
)
    aff_f = one(value_type(typeof(f))) * f
    return build_constraint(_error, aff_f, l, u)
end
...
Convert the coefficients and constants of functions and sets in the `x` to the
...
"""
model_convert(::AbstractModel, x::Any) = x
...
function model_convert(model::AbstractModel, x::Number)
    T = value_type(typeof(model))
    V = variable_ref_type(model)
    C = _complex_convert_type(T, typeof(x))
    return convert(GenericAffExpr{C,V}, x)
end
...
function _constraint_macro(
    ...
    model, y, extra = esc(pos_args[1]), pos_args[2], pos_args[3:end]
    ...
    c, x = if isa(y, Symbol) || isexpr(y, (:vect, :vcat, :ref, :typed_vcat))
    ...
    constraintcall = if vectorized
        # For vectorized constraints, we set every constraint to have the same
        # name.
        :(add_constraint.($model, $buildcall, $new_name_expr))
    else
        :(add_constraint($model, $buildcall, $new_name_expr))
    end
    code = quote
...
macro objective(model, args...)
    ...
    sense = _moi_sense(_error, args[1])
    ...
    code = quote
        ...
        $expr = _replace_zero($(esc(model)), $expr)
        set_objective($(esc(model)), $sense, $expr)
        $expr
    end
...
macro expression(input_args...)
    _error(str...) = _macro_error(:expression, input_args, __source__, str...)
    args, kw_args, requested_container = Containers._extract_kw_args(input_args)
    if !isempty(kw_args)
        _error("unrecognized keyword argument")
    elseif isexpr(args[2], :block)
        _error("Invalid syntax. Did you mean to use `@expressions`?")
    elseif !(2 <= length(args) <= 3)
        _error("needs at least two arguments.")
    end
    expr_name = length(args) == 2 ? gensym() : args[2]
    index_names, indices = Containers.build_ref_sets(_error, expr_name)
    ...
    if _is_anonymous(expr_name)
        return _finalize_and_return(model, code, __source__)
    end
...
# variable name and which is the value if both are symbols. For example, the lhs
# could be the Symbol `:x` and rhs could be the Symbol `:a`, where a variable
# `a` is assigned to 1 in the local scope. Knowing this, we know that `x` is the
# variable name but at parse time there is no way to know that `a` has a value.
...
function parse_one_operator_variable(
    ...
    _set_upper_bound_or_error(_error, info_expr, upper)
    return
end
...
function parse_one_operator_variable(
    ...
    _set_lower_bound_or_error(_error, info_expr, lower)
    return
end
...
macro variable(args...)
    ...
    for ex in extra
        if ex == :Int
            _set_integer_or_error(_error, info_expr)
        elseif ex == :Bin
            _set_binary_or_error(_error, info_expr)
        end
    end
    ...
    creation_code = if isa(var, Symbol) || set !== nothing
