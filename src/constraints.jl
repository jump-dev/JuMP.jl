...
Set the primal start value (MOI attribute `ConstraintPrimalStart`) of the
constraint `con_ref` to `value`. To remove a dual start value set it to
`nothing`.
...
Return the primal start value (MOI attribute `ConstraintPrimalStart`) of the
constraint `con_ref`.
