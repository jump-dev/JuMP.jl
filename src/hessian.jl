...
function compute_hessian_sparsity(s::SymbolicOutput)
    colorlist = [Set()]
    compute_hessian_sparsity(s.tree, 1, colorlist)
    edgelist = {}
...
function compute_hessian_sparsity(x::ExprNode, colorid, colorlist)
    if isexpr(x.ex, :call)
        ...
        if colorid == 1 
            if (x.ex.args[1] == :(+) || x.ex.args[1] == :(-))
                ...
                for i in 2:length(x.ex.args)
                    compute_hessian_sparsity(x.ex.args[i], colorid, colorlist)
                end
            else
                # this is a new f_i, make a new color
                for i in 2:length(x.ex.args)
                    newset = Set()
                    push!(colorlist, newset)
                    newid = length(colorlist)
                    compute_hessian_sparsity(x.ex.args[i], newid, colorlist)
                end
            end
        else
            # give the same color to the children
        ...
        if colorid != 1
            push!(colorlist[colorid], x.ex)
        end
    end
    
end

compute_hessian_sparsity(x, colorid, colorlist) = nothing
