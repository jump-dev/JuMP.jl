type JuMPArray{T,N::Int,R<:OrdinalRange} <: JuMPContainer
...
function Base.getindex{T,N}(d::JuMPArray{T,N,StepRange{Int,Int}},val::T,indices::Int...)
    length(indices) == N || error("Wrong number of indices for ",d.name,", expected ",length(d.indexsets))
    idx = Array(Int, N)
    steps  = 0
    starts = 0
