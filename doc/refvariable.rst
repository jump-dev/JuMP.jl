...
Note that ``setvalue(x, val)`` will not fix the value of a regular variable, it will only set a warmstart for non-fixed variables. One can fix existing, non-fixed variables with ``JuMP.fix(x, val)``.
