...
- New features:
  * Add support for skew symmetric variables via
  ...
  * Add `unregister(model::Model, key::Symbol)` to unregister a name `key` from
    `model`.
  * Add `callback_node_status` for use in callbacks.
  * Add `print_bridge_graph` to visualize the bridging graph generated by
    MathOptInterface.
- Bug fixes:
