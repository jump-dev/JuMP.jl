@time begin
    ...
    *(t1::MyType{S}, t2::MyType{T}) where {S, T} = MyType(t1.a*t2.a)
    ...
    const ModelTypeType = Type{<:JuMP.AbstractModel}
    const VariableRefType = Type{<:JuMP.AbstractVariableRef}
...
function test_basic_operator_overloads(ModelType, VariableRefType)
    m = ModelType()
    @variable(m, w)
    @variable(m, x)
    @variable(m, y)
...
function test_higher_level_operators(ModelType, VariableRefType)
    m = ModelType()
    @testset "sum" begin
        sum_m = ModelType()
...
function test_vectorized_arithmetic(ModelType, VariableRefType)
        m = ModelType()
        @variable(m, x[1:3])
        A = [2 1 0
            1 2 1
...
function test_dot_ops(ModelType, VariableRefType)
        m = ModelType()
        ...
        @test JuMP.isequal_canonical(A.-x, B.-x)
        @test JuMP.isequal_canonical(A.-x, A.-y)
        @test JuMP.isequal_canonical(x .- x, [zero(JuMP.GenericAffExpr{Float64, VariableRefType}) for _1 in 1:2, _2 in 1:2])
        @test JuMP.isequal_canonical(A.-y, B.-y)
        @test JuMP.isequal_canonical(x.-A, [-1+x[1,1]  -2+x[1,2];
                            -3+x[2,1]  -4+x[2,2]])
        @test JuMP.isequal_canonical(x.-A, x.-B)
...
function test_vectorized_comparisons(ModelType, VariableRefType)
        m = ModelType()
        @variable(m, x[1:3])
        A = [1 2 3
            0 4 5
...
function test_vectorized_operations(ModelType, VariableRefType)
    println("vect Transpose")
    @time @testset "Transpose" begin
        m = ModelType()
        @variable(m, x[1:3])
    ...
    println("vect artith")
    @time test_vectorized_arithmetic(ModelType, VariableRefType)

    println("vect dot ops")
    @time test_dot_ops(ModelType, VariableRefType)

    println("vect comps")
    @time test_vectorized_comparisons(ModelType, VariableRefType)
end


function test_operators_for_nonarray_abstractarrays(ModelType)
    m = ModelType()
    @variable(m, x[1:3])
...
function test_diagm_for_nonarray_abstractarrays(ModelType)
    m = ModelType()
    @variable(m, x[1:3])
...
function test_vector_matrix_multiplication_with_custom_types(ModelType, VariableRefType)
    m = ModelType()
...
function test_operator_warn(ModelType)
    m = ModelType()
    @variable m x[1:51]
...
function operators_test(ModelType::Type{<:JuMP.AbstractModel}, VariableRefType::Type{<:JuMP.AbstractVariableRef})
    println("Promotion")
    @time test_promotion(ModelType, VariableRefType)

    println("Basic operator overloads")
    @time test_basic_operator_overloads(ModelType, VariableRefType)

    println("Higher-level operators")
    @time test_higher_level_operators(ModelType, VariableRefType)

    println("Vectorized operations")
    @time test_vectorized_operations(ModelType, VariableRefType)

    println("Operators for non-Array AbstractArrays")
    @time test_operators_for_nonarray_abstractarrays(ModelType)

    println("diagm for non-Array AbstractArrays")
    @time test_diagm_for_nonarray_abstractarrays(ModelType)

    println("DimensionMismatch when performing vector-matrix multiplication with custom types #988")
    @time test_vector_matrix_multiplication_with_custom_types(ModelType,
                                                              VariableRefType)

    println("operator_warn")
    @time test_operator_warn(ModelType)
end
