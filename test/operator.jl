...
function operators_test(ModelType::Type{<:JuMP.AbstractModel}, VariableRefType::Type{<:JuMP.AbstractVariableRef})
    ...
    @testset "Vectorized operations" begin
        ...
        @testset "Vectorized arithmetic" begin
            ...
            @test JuMP.isequal_canonical(A*x, JuMP.@_expression(B*x))
            @test JuMP.isequal_canonical(JuMP.@_expression(A*x), JuMP.@_expression(B*x))
            @test JuMP.isequal_canonical(x'*A, [2x[1]+x[2]; 2x[2]+x[1]+x[3]; x[2]+2x[3]]')
            @test JuMP.isequal_canonical(x'*A, x'*B)
            @test JuMP.isequal_canonical(x'*A, JuMP.@_expression(x'*B))
            @test JuMP.isequal_canonical(JuMP.@_expression(x'*A), JuMP.@_expression(x'*B))
            @test JuMP.isequal_canonical(x'*A*x, 2x[1]*x[1] + 2x[1]*x[2] + 2x[2]*x[2] + 2x[2]*x[3] + 2x[3]*x[3])
            @test JuMP.isequal_canonical(x'A*x, x'*B*x)
            @test JuMP.isequal_canonical(x'*A*x, JuMP.@_expression(x'*B*x))
            @test JuMP.isequal_canonical(JuMP.@_expression(x'*A*x), JuMP.@_expression(x'*B*x))
            ...
            @test JuMP.isequal_canonical(JuMP.@_expression(A*x/2), A*x/2)
        ...
        @testset "Vectorized comparisons" begin
            ...
            @test JuMP.isequal_canonical((x'A)' + 2A*x, JuMP.@_expression((x'A)' + 2A*x))
            @test JuMP.isequal_canonical((x'A)' + 2A*x, JuMP.@_expression((x'B)' + 2A*x))
            @test JuMP.isequal_canonical((x'A)' + 2A*x, JuMP.@_expression((x'A)' + 2B*x))
            @test JuMP.isequal_canonical((x'A)' + 2A*x, JuMP.@_expression((x'B)' + 2B*x))
