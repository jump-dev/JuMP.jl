...
cpx && push!(lp_solvers, CPLEX.CplexSolver())
...
cpx && push!(ip_solvers, CPLEX.CplexSolver())
mos && push!(ip_solvers, Mosek.MosekSolver())
...
cpx && push!(semi_solvers, CPLEX.CplexSolver())
# SOS solvers
sos_solvers = Any[]
grb && push!(sos_solvers, Gurobi.GurobiSolver(OutputFlag=0))
cpx && push!(sos_solvers, CPLEX.CplexSolver())
...
if cpx
    push!(lazy_solvers, CPLEX.CplexSolver(CPX_PARAM_PRELINEAR=0, CPX_PARAM_PREIND=0, CPX_PARAM_ADVIND=0, CPX_PARAM_MIPSEARCH=1,CPX_PARAM_MIPCBREDLP=0))
    push!( cut_solvers, CPLEX.CplexSolver(CPX_PARAM_PRELINEAR=0, CPX_PARAM_PREIND=0, CPX_PARAM_ADVIND=0, CPX_PARAM_MIPSEARCH=1,CPX_PARAM_MIPCBREDLP=0))
    push!(heur_solvers, CPLEX.CplexSolver(CPX_PARAM_PRELINEAR=0, CPX_PARAM_PREIND=0, CPX_PARAM_ADVIND=0, CPX_PARAM_MIPSEARCH=1,CPX_PARAM_MIPCBREDLP=0)) 
end
if glp
...
cpx && push!(quad_solvers, CPLEX.CplexSolver())
mos && push!(quad_solvers, Mosek.MosekSolver())
# Nonlinear solvers
nl_solvers = Any[]
...
mos && push!(nl_solvers, Mosek.MosekSolver())
