...
@testset "Generation and solve with fake solver" begin
    ...
    @testset "ResultCount" begin
        ...
        @test 1.0 == @inferred JuMP.value(x + y, result = 1)
        @test 0.0 == @inferred JuMP.value(x * y, result = 1)
        ...
        @test 1.0 == @inferred JuMP.value(x + y, result = 2)
        @test 0.0 == @inferred JuMP.value(x * y, result = 2)
        ...
        @test_throws MOI.ResultIndexBoundsError JuMP.value(x + y, result = 3)
        @test_throws MOI.ResultIndexBoundsError JuMP.value(x * y, result = 3)
