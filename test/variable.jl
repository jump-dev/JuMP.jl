...
function sliceof(x, I, J, K)
    ...
    @testset "Key iterators of JuMPContainers" begin
        ...
        @test all(reshape(collect(keys(x)), :) .== [(2,1), (3,1), (2,2), (3,2)])
        @test all(reshape(collect(keys(y)), :) .== [(2,), (3,)])
        @test all(reshape(collect(keys(z)), :) .== [(-1,:red), (0,:red), (1,:red), (-1,"blue"), (0,"blue"), (1,"blue")])
