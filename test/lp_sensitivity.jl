...
function test_lp_rhs_perturbation_range(
    model_string,
    primal_solution,
    basis_status,
    feasibility_ranges,
    ...
    for (variable_name, value) in primal_solution
        ...
        MOI.set(
            mock_optimizer,
            MOI.VariablePrimal(),
            JuMP.optimizer_index(variable_ref),
            value,
        )
    end
    ...
    @testset "rhs range of $constraint_name" for constraint_name in
                                                 keys(feasibility_ranges)
        ci = MOI.get(JuMP.backend(model), MOI.ConstraintIndex, constraint_name)
        constraint_ref = JuMP.ConstraintRef(model, ci, JuMP.ScalarShape())
...
function test_lp_objective_perturbation_range(
    model_string,
    dual_solution,
    basis_status,
    optimality_ranges,
    ...
    @testset "optimality range of $variable_name" for variable_name in
                                                      keys(optimality_ranges)
        vi = MOI.get(JuMP.backend(model), MOI.VariableIndex, variable_name)
        variable_ref = JuMP.VariableRef(model, vi)
