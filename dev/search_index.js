...

{
    ...
    "title": "Installation Guide",
    "category": "section",
    "text": "JuMP is a package for Julia. To use JuMP, first download and install Julia or open up a remote notebook at JuliaBox or similar services.This version of JuMP is compatible with Julia 1.0 and later.From Julia, JuMP is installed by using the built-in package manager:import Pkg\nPkg.add(\"JuMP\")"
},
...
{
    "location": "installation/#CDD-1",
    "page": "Installation Guide",
    "title": "CDD",
    "category": "section",
    "text": "CDD can solve the problem both using Float64 and Rational{BigInt} arithmetics. The arithmetic used the type T given in CDDLib.Optimizer{T}. Only CDDLib.Optimizer{Float64} can be used with JuMP as JuMP inputs the problem in Float64 arithmetics. Use MOI directly for CDDLib.Optimizer{Rational{BigInt}}."
},
...

{
    ...
    "category": "section",
    "text": "To modify the coefficients for a linear term in a constraint (but notably not yet the coefficients on a quadratic term), use set_normalized_coefficient. To query the current coefficient, use normalized_coefficient.julia> @constraint(model, con, 2x[1] + x[2] <= 1)\ncon : 2 x[1] + x[2] ≤ 1.0\n\njulia> set_normalized_coefficient(con, x[2], 0)\n\njulia> con\ncon : 2 x[1] ≤ 1.0\n\njulia> normalized_coefficient(con, x[2])\n0.0note: Note\nJuMP normalizes constraints into a standard form by moving all terms involving variables onto the left-hand side of the constraint.@constraint(model, 2x <= 1 - x)will be normalized to@constraint(model, 3x <= 1)set_normalized_coefficient sets the coefficient of the normalized constraint."
},
...

{
    ...
    "category": "section",
    "text": "The container function encodes the logic for how containers are constructed in JuMP\'s macros. The @container macro is available to create containers independently of any JuMP model.JuMP.Containers.container\nJuMP.Containers.default_container\nJuMP.Containers.VectorizedProductIterator\nJuMP.Containers.NestedIterator\nJuMP.Containers.@containerIn the @variable (resp. @constraint) macro, containers of variables (resp. constraints) can be created with the following syntax:name[index_set_1, index_set_2, ..., index_set_n] creating an n-dimensional container of name name; or\n[index_set_1, index_set_2, ..., index_set_n] creating an anonymous n-dimensional container.Each expression index_set_i can either beof the form index_set specifying that the ith index set of the container is index_set; or\nof the form index_name=index_set specifying that the ith index set of the container is index_set and allowing values used in the macro expression and keyword arguments to be expressions depending on the index_name.The macro then creates the container using the JuMP.Containers.container function with the following arguments:A function taking as argument the value of the indices and returning the value to be stored in the container, e.g. a variable for the @variable macro and a constraint for the @constraint macro.\nAn iterator over the indices of the container.\nThe value of the container keyword argument if given."
},
