...

{
    ...
    "category": "section",
    "text": "Provided the primal status is not (MOI.NO_SOLUTION), the primal solution can be obtained by calling value. For the dual solution, the function is dual. An equivalent way to check if the status is not MOI.NO_SOLUTION is by calling has_values for the primal status and has_duals for the dual solution.It is important to note that if has_values or has_duals  return false, calls to value and dual might throw an error  or return arbitrary values.The container type (e.g., scalar, vector, or matrix) of the returned solution (primal or dual) depends on the type of the variable or constraint. See AbstractShape and dual_shape for details.To call value or dual on container of VariableRef or ConstraintRef, use the broadcast syntax, e.g., value.(x).The objective value of a solved problem can be obtained via objective_value. The best known bound on the optimal objective value can be obtained via objective_bound.A recommended workflow for solving a model and querying the solution is the following:using JuMP\nmodel = Model()\n@variable(model, x[1:10] >= 0)\n# ... other constraints ...\noptimize!(model)\n\nif termination_status(model) == MOI.OPTIMAL\n    optimal_solution = value.(x)\n    optimal_objective = objective_value(model)\nelseif termination_status(model) == MOI.TIME_LIMIT && has_values(model)\n    suboptimal_solution = value.(x)\n    suboptimal_objective = objective_value(model)\nelse\n    error(\"The model was not solved correctly.\")\nend# TODO: How to accurately measure the solve time."
},
