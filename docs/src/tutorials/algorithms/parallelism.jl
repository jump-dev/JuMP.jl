...
function an_incorrect_way_to_use_threading()
...
function dont_run_segfault_likely()
...
function an_incorrect_way_to_build_with_multithreading()
    model = Model()
...
#
#     nested task error: ConcurrencyViolationError("Vector can not be resized concurrently")
# ```

# The error happens because JuMP models are not thread-safe. Code that uses
# multi-threading to simultaneously modify or optimize a single JuMP model
# across threads may error, crash Julia, or silently produce incorrect results.
...
a_correct_way_to_build_with_multithreading()
