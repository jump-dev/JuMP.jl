...
ERROR: At REPL[36]:1: `@constraint(model, X >= 0)`: Unsupported matrix in vector-valued set. Did you mean to use the broadcasting syntax `.>=` instead? Alternatively, perhaps you are missing a set argument like `@constraint(model, X >= 0, PSDCone())` or `@constraint(model, X >= 0, HermmitianPSDCone())`.
Stacktrace:
[...]
```

Instead, to represent matrix inequalities you must always use the broadcasting
`.==`, `.>=`, or `.<=`, or use the [Set inequality syntax](@ref).

There are two exceptions.

First, if both sides are symmetric, you may use the non-broadcasting equality
syntax:
...
You can also use `==` if both sides are Hermitian, which again avoids adding the
redundant constraints that are added when broadcasting with `.==`:
