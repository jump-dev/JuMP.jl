...
function add_tutorial_overview()
    package_to_headers = Dict{String,Vector{String}}()
    for (root, dirs, files) in walkdir(joinpath(@__DIR__, "src", "tutorials"))
...
function validate_pages(pages, source)
    set = Set{String}()
    for page in pages
        _add_to_set(set, page)
    end
    missing_files = String[]
    doc_src = joinpath(source, "")
...
function list_of_solvers_and_extensions()
    ...
    return _LIST_OF_SOLVERS, _LIST_OF_EXTENSIONS
end

function get_moi_pages()
    moi_docs = joinpath(pkgdir(MOI), "docs")
    ...
    return "MathOptInterface" => eval(Meta.parse(s))
end
...
function documentation_structure()
    _LIST_OF_SOLVERS, _LIST_OF_EXTENSIONS = list_of_solvers_and_extensions()
        ...
        modules = [
            ...
            JuMP.Containers => [
                ...
                "Containers.@container" =>
                    DocumenterReference.DOCTYPE_MACRO,
                "Containers.container" =>
                    DocumenterReference.DOCTYPE_FUNCTION,
                "Containers.rowtable" =>
                    DocumenterReference.DOCTYPE_FUNCTION,
    ...
    pages = [
        "Introduction" =>
            ["index.md", "should_i_use.md", "installation.md"],
        "Tutorials" => [
...
function make_html()
    ...
    p = pop!(pages)
    push!(pages, get_moi_pages())
    push!(pages, p)
    validate_pages(pages, joinpath(@__DIR__, "src"))
    # Create remotes for Documenter
    status = sprint(io -> Pkg.status("MathOptInterface"; io))
    version = match(r"(v[0-9].[0-9]+.[0-9]+)", status)[1]
...
function make_latex()
    ...
    for (root, dirs, files) in walkdir(tutorial_dir)
        for dir in dirs
            files = readdir(joinpath(root, dir); join = true)
    ...
    pop!(pages)             # remove release_notes.md
    pop!(pages)             # remove /Extensions
    pop!(pages)             # remove /Solvers
    moi_pages = get_moi_pages()
    pop!(moi_pages[2])      # remove /MathOptInterface/release_notes.md
    push!(pages, moi_pages) # Re-add /MathOptInterface
    section_title, contents = pages[4]
    ...
    pages[4] = section_title => ["Docstrings" => contents]
    Documenter.makedocs(;
        sitename = "JuMP",
        authors = "The JuMP core developers and contributors",
